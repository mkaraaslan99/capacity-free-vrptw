...
% Date and Institution at top
{\large December 4, 2025 \hfill Hacettepe University}
...
\vspace{0.3cm}
\noindent\textbf{Code Availability:} The complete implementation of the proposed framework, including all algorithms, experimental scripts, and datasets, is publicly available on GitHub: \url{https://github.com/mkaraaslan99/capacity-free-vrptw}. Detailed instructions for running the code and reproducing the experimental results are provided in the \texttt{HOWTORUN.txt} file in the repository.
...
\textbf{detailed\_runs\_report.py} -- Generates detailed stage-by-stage performance reports with per-run and summary statistics in CSV format.

\textbf{export\_report\_tables.py} -- Exports formatted tables from experimental results for inclusion in research reports. Converts detailed run data into pivoted wide-format tables (per-run cost, per-run routes) and summary statistics tables with mean, standard deviation, min, max values for each algorithm variant and instance.

\textbf{stage\_by\_stage\_report.py} -- Generates comprehensive stage-by-stage analysis showing algorithm progression through construction, 2-opt, and relocation phases. Exports results to CSV with detailed metrics for each algorithmic stage.

\textbf{plot\_detailed\_runs\_figures.py} -- Creates visualization figures including cost distributions, runtime analysis, and route count plots from CSV experimental results.
...
\textbf{duration\_sweep.py} -- Performs sensitivity analysis by sweeping maximum route duration parameters and exports results to CSV format for analysis.
